...
func args2config() (tail.Config, int64) {
	...
	flag.BoolVar(&config.Follow, "f", true, "wait for additional data to be appended to the file")
	flag.BoolVar(&config.ReOpen, "F", true, "follow, and track file rename/rotation")
	flag.BoolVar(&config.Poll, "p", true, "use polling, instead of inotify")
	flag.Parse()
	if config.ReOpen {
		config.Follow = true
	}
